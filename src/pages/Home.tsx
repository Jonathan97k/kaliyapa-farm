import { ArrowRight, CheckCircle2, Quote, Loader2 } from 'lucide-react';
import { motion } from 'motion/react';
import { Link } from 'react-router-dom';
import Hero from '../components/Hero';
import { SERVICES as STATIC_SERVICES, TESTIMONIALS, FARM_NAME, CONTACT_PHONE } from '../constants';
    ...
    async function fetchServices() {
      try {
        ...
        if (!querySnapshot.empty) {
          ...
          // Sort by title or use the first 3 for the bento grid
          ...
          <div className="bento-grid">
                ...
                >
                  <div className="h-full flex flex-col">
                    <div className="h-[50%] md:h-[55%] overflow-hidden">
                      <img src={services[0]?.image || STATIC_SERVICES[0].image} className="w-full h-full object-cover img-zoom" alt={services[0]?.title || STATIC_SERVICES[0].title} />
                    </div>
                    <div className="p-5 md:p-7 lg:p-10 flex flex-col justify-between flex-grow">
                      <div>
                        <h3 className="text-2xl md:text-3xl lg:text-4xl font-serif font-bold text-primary mb-2 md:mb-3">{services[0]?.title || STATIC_SERVICES[0].title}</h3>
                        <p className="text-primary/60 text-sm md:text-base lg:text-lg font-light mb-4 md:mb-6 max-w-sm line-clamp-2">{services[0]?.description || STATIC_SERVICES[0].description}</p>
                      </div>
            ...
            >
              <div className="flex h-full flex-col sm:flex-row">
                <div className="w-full sm:w-1/2 p-5 md:p-8 flex flex-col justify-center">
                  <h3 className="text-xl md:text-2xl lg:text-3xl font-serif font-bold text-secondary mb-2 md:mb-3">{services[1]?.title || STATIC_SERVICES[1].title}</h3>
                ...
                <div className="w-full sm:w-1/2 h-40 sm:h-auto overflow-hidden">
                  <img src={services[1]?.image || STATIC_SERVICES[1].image} className="w-full h-full object-cover opacity-80 img-zoom" alt={services[1]?.title || STATIC_SERVICES[1].title} />
                </div>
              </div>
            </motion.div>
            ...
            >
              <div className="flex h-full flex-col sm:flex-row-reverse">
                <div className="w-full sm:w-1/2 p-5 md:p-8 flex flex-col justify-center">
                  <h3 className="text-xl md:text-2xl lg:text-3xl font-serif font-bold text-primary mb-2 md:mb-3">{services[2]?.title || STATIC_SERVICES[2].title}</h3>
                ...
                <div className="w-full sm:w-1/2 h-40 sm:h-auto overflow-hidden">
                  <img src={services[2]?.image || STATIC_SERVICES[2].image} className="w-full h-full object-cover img-zoom" alt={services[2]?.title || STATIC_SERVICES[2].title} />
                </div>
              </div>
            </motion.div>
              </>
                   ...
                   >
                     <div className="flex items-center gap-3 md:gap-4 mb-6 md:mb-8">
                        ...
                        <div>
                           ...
                           <div className="text-[10px] font-bold tracking-[0.2em] uppercase opacity-40">{t.role}</div>
                        </div>
                     </div>
                     <Quote className="text-secondary/10 h-8 md:h-12 mb-4 md:mb-6" strokeWidth={1} />
                     <p className="text-lg md:text-xl lg:text-2xl font-serif italic text-primary/80 leading-relaxed">"{t.text}"</p>
                   </motion.div>
                 ))}
              </div>
           </div>
