...
export default async function handler(request: Request) {
  ...
  if (request.method === 'POST') {
    try {
      ...
      const { name, role, text, image } = validateTestimonialInput(body);
      const id = crypto.randomUUID();

      await turso.execute({
        sql: 'INSERT INTO testimonials (id, name, role, text, image) VALUES (?, ?, ?, ?, ?)',
        args: [id, name, role, text, image || null],
      });
      ...
      return new Response(JSON.stringify({ error: (error as Error).message || 'Failed to add testimonial' }), {
        status: 400,
        headers: { 'Content-Type': 'application/json' },
      });
    }
  ...
  if (request.method === 'PUT') {
    try {
      ...
      await turso.execute({
        sql: 'UPDATE testimonials SET name = ?, role = ?, text = ?, image = ? WHERE id = ?',
        args: [name, role, text, image || null, id],
      });
      ...
      return new Response(JSON.stringify({ error: (error as Error).message || 'Failed to update testimonial' }), {
        status: 400,
        headers: { 'Content-Type': 'application/json' },
      });
    }
